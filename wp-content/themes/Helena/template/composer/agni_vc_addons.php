	...
	function agni_space_atts_processor( $atts ){
		...
		foreach ($device as $key => $value) {
			...
			if( !empty($atts['margin_top'.$value]) ){
				$design_css[$key] .= 'margin-top: ' . ( preg_match( '/(px|em|\%|pt|cm)$/', $atts['margin_top'.$value] ) ? $atts['margin_top'.$value] : $atts['margin_top'.$value] . 'px' ) . '; ';
			}
			if( !empty($atts['margin_right'.$value]) ){
				$design_css[$key] .= 'margin-right: ' . ( preg_match( '/(px|em|\%|pt|cm)$/', $atts['margin_right'.$value] ) ? $atts['margin_right'.$value] : $atts['margin_right'.$value] . 'px' ) . '; ';
			}
			if( !empty($atts['margin_bottom'.$value]) ){
				$design_css[$key] .= 'margin-bottom: ' . ( preg_match( '/(px|em|\%|pt|cm)$/', $atts['margin_bottom'.$value] ) ? $atts['margin_bottom'.$value] : $atts['margin_bottom'.$value] . 'px' ) . '; ';
			}
			if( !empty($atts['margin_left'.$value]) ){
				$design_css[$key] .= 'margin-left: ' . ( preg_match( '/(px|em|\%|pt|cm)$/', $atts['margin_left'.$value] ) ? $atts['margin_left'.$value] : $atts['margin_left'.$value] . 'px' ) . '; ';
			}
...
if( !function_exists( 'agni_carousel_atts_processor' ) ){
	function agni_carousel_atts_processor( $atts ){
		...
		$carousel_atts = 'data-carousel-slide-to-show="'.esc_attr( $atts['columns'] ).'" data-carousel-slide-to-show-1200="'.esc_attr( $atts['carousel_columns_1200'] ).'" data-carousel-slide-to-show-992="'.esc_attr( $atts['carousel_columns_992'] ).'" data-carousel-slide-to-show-768="'.esc_attr( $atts['carousel_columns_768'] ).'" data-carousel-autoplay="'.esc_attr( $carousel_autoplay ).'" data-carousel-autoplay-speed="'.esc_attr( $carousel_autoplay_speed ).'" data-carousel-speed="'.esc_attr( $carousel_speed ).'" data-carousel-pause-on-focus="'.esc_attr( $carousel_pause_on_focus ).'" data-carousel-infinite="'.esc_attr( $carousel_infinite ).'" data-carousel-dots="'.esc_attr( $carousel_dots ).'" data-carousel-arrows="'.esc_attr( $carousel_arrows ).'" data-carousel-draggable="'.esc_attr( $carousel_draggable ).'" data-carousel-center-mode="'.esc_attr( $carousel_center_mode ).'" data-carousel-swipe="'.esc_attr( $carousel_swipe ).'" data-carousel-swipe-to-scroll="'.esc_attr( $carousel_swipe_to_scroll ).'"';
...
add_action( 'init', 'agni_vc_shortcode_values_update' );
...
class AgniShortcodesFunctions {
	...
	*/
	...
	public static function agni_gallery( $atts = null, $content = null ) {
		...
		$carousel_nav = $carousel_atts = $carousel_container_class = $gallery_class = $columns = $gallery_autoplay = $gallery_autoplay_timeout = $gallery_autoplay_hover = $gallery_loop = $gallery_autoheight = $gallery_center = $gallery_pagination = $gallery_row_attr = $gallery_column_padding = $gallery_row_margin = $gallery_lightbox = $grayscale = $img = $img_array = $gallery_grid_sizer = '';
		...
		$output = '<div class="agni-gallery '.$atts['carousel_type'].' '.$gallery_lightbox.' '.$carousel_container_class.'"><div class="agni-gallery-row row '.$gallery_class.' '. $atts['class'].'" '.$gallery_row_attr.$carousel_atts.' '.$gallery_row_margin.'>'.$gallery_grid_sizer.$src1.'</div>'.$carousel_nav.'</div>';
	...
	public static function agni_category_box( $atts = null, $content = null ) {
		...
		$output .= '<div class="agni-category-box-image '.$has_fullwidth_img.'">'.$img.'</div>
	...
	public static function agni_hotspot( $atts = null, $content = null ) {
		...
		foreach ( $data as $atts ) {
			...
			if( $atts['pin_content'] == 'product' ){
				...
				$hotspot_product = new WC_Product( $hotspot_product_id );
