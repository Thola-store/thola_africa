...
class halena_latest_works extends WP_Widget {
	...
	public function __construct(){
		...
		parent::__construct(
			'halena_latest_works',
			esc_html__( 'Halena: Latest Works', 'halena' ),
				array(
					'classname'   => 'widget_halena_latest_works',
					'description' => esc_html__( 'Your site\'s most recent works with unique layout of halena. You can also display the latest works of particular category.', 'halena' )
				)
			);			
	...
	public function form( $instance ) {
		$defaults = array( 'title' => esc_html__('Latest Works', 'halena'), 'number' => 5, 'categories' => '');
		...
		<p>
            ...
            <select id="<?php echo esc_attr( $this->get_field_id( 'categories' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'categories' ) ); ?>" class="widefat" style="width:100%;">
				<option value='' <?php selected( $instance['categories'], '' ); ?>><?php esc_html_e('All Categories', 'halena'); ?></option>
