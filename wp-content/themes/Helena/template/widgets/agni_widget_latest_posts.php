...
class halena_latest_posts extends WP_Widget {
	...
	public function __construct(){
		...
		parent::__construct(
			'halena_latest_posts',
			esc_html__( 'Halena: Latest Posts', 'halena' ),
				array(
					'classname'   => 'widget_halena_latest_posts',
					'description' => esc_html__( 'Your site\'s most recent posts with unique layout of halena. You can also display the latest posts of particular category.', 'halena' )
				)
			);			
	...
	public function form( $instance ) {
		$defaults = array( 'title' => esc_html__('Latest Posts', 'halena'), 'number' => 5, 'categories' => '');
		...
		<p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'categories' ) ); ?>"><?php esc_html_e('Show by category:', 'halena'); ?></label> 
            <select id="<?php echo esc_attr( $this->get_field_id( 'categories' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'categories' ) ); ?>" class="widefat" style="width:100%;">
				<option value='all' <?php selected( $instance['categories'], 'all' ); ?>><?php esc_html_e('All Categories', 'halena'); ?></option>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php esc_html_e('Number of posts to show:', 'halena'); ?></label>
