...
function halena_scripts() {
    ...
    $gmap_api = (!empty($halena_options['gmap-api']))?'?key='.$halena_options['gmap-api']:'';
    ...
    if( !empty($halena_options['google-font-additional']) ){
        wp_enqueue_style( 'halena-google-fonts-additional', '//fonts.googleapis.com/css?family='.$halena_options['google-font-additional'] );
    }
...
{
    if ( !empty ( $GLOBALS['admin_page_hooks']['halena'] ) ){
        global $submenu;

        $temp = $submenu['halena'][1];
        $submenu['halena'][1] = $submenu['halena'][2];
        $submenu['halena'][2] = $temp;
...
function agni_admin_menu_bar( $wp_admin_bar ){
    if( current_user_can( 'edit_theme_options' ) ){
        $args = array(
            'id' => 'halena-admin-menu',
            'title' => 'Halena',
            'href' => admin_url() . 'admin.php?page=halena', 
            'meta' => array(
                'class' => 'halena-admin-menu', 
            ),
        );
...
// Header Logo SVG
function agni_header_svg_logo(){
    global $halena_options;

    echo agni_extract_svg_from_file( $halena_options['logo-1']['url'], 'header-logo-icon-svg' );
}
        ...
        <?php  if(!empty($halena_options['logo-1']['url']) && $halena_options['header-site-title'] == '0'){  ?>
            <a href="<?php echo esc_url( home_url( '/' ) ); ?>" class="logo-icon <?php echo esc_attr($logo_1_class); ?>">
                ...
                if( agni_detect_is_svg_file( $header_logo_src ) ){ ?>
                    <span class="logo-icon-svg"><?php do_action( 'agni_header_svg_logo' ); ?></span>
                <?php } 
                else { ?>
    ...
    function agni_header_lang_box(){
        ...
        if( $halena_options['header-lang-box'] == '1' ){ ?>
            <div class="header-toggle header-lang-toggle">
                <span><?php //echo htmlspecialchars_decode( esc_html($halena_options['header-lang-name']) ); ?></span>
                <?php
                $header_wishlist_svg_icon = AGNI_FRAMEWORK_URL . '/img/halena_lang_icon.svg'; 
                echo agni_extract_svg_from_file( $header_wishlist_svg_icon, $unique_class = 'header-wishlist-icon-svg' );

                echo htmlspecialchars_decode( esc_html($halena_options['header-lang-list']) ); ?>
            </div>
        <?php } 
    }
        ...
        function agni_woocommerce_ajax_myaccount(){
            ...
            if( $halena_options['header-myaccount-box'] == '1' ){ ?>
                <div class="header-myaccount-toggle header-toggle">
                    <a class="header-myaccount-icon-url" href="<?php echo get_permalink( get_option('woocommerce_myaccount_page_id') ); ?>"><span>
                        <?php $header_myaccount_svg_icon = AGNI_FRAMEWORK_URL . '/img/halena_myaccount_icon.svg'; 
...
if(class_exists( 'WooCommerce' )){ 
    if( !function_exists('agni_woocommerce_ajax_cart') ){
        function agni_woocommerce_ajax_cart(){ 
            ...
            if( $halena_options['header-cart-box'] == '1' ){ ?>
                <div class="header-cart-toggle header-toggle">
                    <span class="cart-contents">
                        <span class="header-cart-icon">
                            <?php if( $halena_options['header-cart-style'] == '2' ){
                                $header_cart_svg_icon = AGNI_FRAMEWORK_URL . '/img/halena_shopping_cart_icon.svg'; 
                                echo agni_extract_svg_from_file( $header_cart_svg_icon, $unique_class = 'header-cart-icon-svg' ); 
                            } 
                            else { ?>
                                <span class="header-cart-text"><?php echo esc_html( 'Cart', 'halena' ); ?></span>
                            <?php } ?>
                        </span>
        ...
        function agni_woocommerce_wishlist_icon(){ 
            ...
            if( $halena_options['header-wishlist-box'] == '1' ){ ?>
                <div class="header-wishlist-toggle header-toggle">
                    <a class="wishlist-url" href="<?php echo YITH_WCWL()->get_wishlist_url(); ?>"><span>
                        <?php $header_wishlist_svg_icon = AGNI_FRAMEWORK_URL . '/img/halena_wishlist_icon.svg'; 
                        echo agni_extract_svg_from_file( $header_wishlist_svg_icon, $unique_class = 'header-wishlist-icon-svg' ); ?>
                    </a>
                </div>
...
if( !function_exists('agni_header_search') ){
        ...
        <div class="header-search search-invisible">
            ...
            <?php }
                ...
                <form id="search-form" role="search" method="get" class="search-form" action="<?php echo esc_url( home_url( '/' ) ); ?>">
                    <label class="screen-reader-text" for="s"><?php esc_html_e( 'Search for:', 'halena' ); ?></label>
    ...
    add_action( 'agni_header_search', 'agni_header_search' );
}
    ...
    function agni_woocommerce_search_icon(){ 
        ...
        if( $halena_options['header-search-box'] == '1' ){ ?>
            <div class="header-search-toggle header-toggle">
                <?php $header_search_svg_icon = AGNI_FRAMEWORK_URL . '/img/halena_search_icon.svg'; 
                echo agni_extract_svg_from_file( $header_search_svg_icon, $unique_class = 'header-search-icon-svg' ); ?>
            </div>
            <?php do_action('agni_header_search'); 
        }
    }
    ...
    function agni_header_social_media_icons(){
        ...
        if( $halena_options['social-media-header'] == '1' && $halena_options['social-media-header-location'] == '1' ){ ?>
            <div class="header-toggle header-social-toggle text-center">
                <?php $header_social_svg_icon = AGNI_FRAMEWORK_URL . '/img/halena_social_media.svg'; 
                        ...
                        <div class="header-navigation-menu-container <?php echo esc_attr( $halena_options['header-menu-style'] ).'-container'; ?> ">
                            <?php switch ($halena_options['header-menu-style']) {
                                        ...
                                        <div class="container<?php if( $header_menu_fullwidth == '1'){ echo '-fluid'; } ?>">
                                            <div class="header-center-menu-top-flex">
                                                ...
                                                <div class="header-menu-icons-container">
                                                    ...
                                                    <div class="header-menu-icons <?php !empty($halena_options['header-icon-link-color-2'])?'header-menu-icons-additional-color':'' ?>">
                                ...
                                default : ?>
                                    <div class="header-menu-content">
                                        <div class="container<?php if( $header_menu_fullwidth == '1'){ echo '-fluid'; } ?>">
                                            <div class="header-menu-flex <?php echo ( !empty($halena_options['header-menu-style-default-choice'] ))?$halena_options['header-menu-style-default-choice']:'right'; ?>-menu-flex <?php echo esc_attr( $halena_options['header-menu-style-choice-order'] ); ?>">
...
if( !function_exists('agni_footer') ){
    function agni_footer(){ 
        ...
        <footer class="site-footer<?php if( $halena_options['footer-sticky'] == '1' ){ echo ' has-sticky-footer'; } ?>" role="contentinfo">
            <div class="site-info">
                <?php if( $footer_bar == '1' ){ ?>
                    <div id="footer-bar-area" class="footer-bar footer-bar-has-<?php echo ( $footer_bar_choice == '0' )?'widget':'content-block'; ?>">
...
if( !function_exists('agni_404') ){
    function agni_404(){
        ...
        if( $halena_options['404-choice'] == '1' && !empty($halena_options['404-contentblock-choice']) ){
            echo do_shortcode('[agni_block post_id="'.$halena_options['404-contentblock-choice'].'"]');
        }
        else{
...
if( !function_exists('agni_page_header') ){
    function agni_page_header( $post, $term = null ){
        ...
        $page_header_bg_choice = !empty($meta_fn( $post, 'page_header_bg_choice', true ))?esc_attr( $meta_fn( $post, 'page_header_bg_choice', true ) ):'bg_image';
        ...
        if( !empty( $page_header_bg_image ) || $page_header_bg_choice != 'bg_image' ){
            ...
            if ( !empty( $page_header_title ) ){
                ...
                $page_header_title = '<div class="agni-slide-title '.$page_header_animation.' '.$page_header_title_font.'" style="font-size:'.$page_header_title_size.'px; color:'.$page_header_title_color.'; margin-bottom:'.( preg_match( '/(px|em|\%|pt|cm)$/', $page_header_title_margin_bottom ) ? $page_header_title_margin_bottom : $page_header_title_margin_bottom . 'px' ).'; '.$page_header_animation_delay.'"><h2 '.$page_header_title_effect.'>'.htmlspecialchars_decode( $page_header_title ).'</h2></div>';
    ...
    function agni_slider( $post, $shortcode = null ){
        ...
        switch( $agni_slider_choice ){
            ...
            case 'slideshow' :
                ...
                $slides = $slideshow_animation = $slider_height = $slide_parallax = $slideshow_fullwidth_container = $slider_css = $slider_class = '';
                ...
                foreach( (array) $slideshow_repeatable as $key => $slide ){
                    ...
                    if ( !empty( $slide['slideshow_desc'] ) ){
                        ...
                        $slideshow_desc = '<div class="agni-slide-description '.$slideshow_animation.' '.$slideshow_desc_font.'" style="font-size:'.$slideshow_desc_size.'px; color:'.$slideshow_desc_color.'; margin-bottom:'.( preg_match( '/(px|em|\%|pt|cm)$/', $slideshow_desc_margin_bottom ) ? $slideshow_desc_margin_bottom : $slideshow_desc_margin_bottom . 'px' ).'; '.$slideshow_animation_delay.'"><p>'.htmlspecialchars_decode( esc_attr( $slide['slideshow_desc'] ) ).'</p></div>';
                    }
            ...
            case 'textslider':
                ...
                foreach( (array) $textslider_repeatable as $key => $slide ){
                    ...
                    if ( !empty( $slide['textslider_title'] ) ){
                        ...
                        $textslider_title = '<div class="agni-slide-title '.$textslider_animation.' '.$textslider_title_font.'" style="font-size:'.$textslider_title_size.'px; color:'.$textslider_title_color.'; margin-bottom:'.( preg_match( '/(px|em|\%|pt|cm)$/', $textslider_title_margin_bottom ) ? $textslider_title_margin_bottom : $textslider_title_margin_bottom . 'px' ).'; '.$textslider_animation_delay.'"><h2 '.$textslider_title_effect.'>'.htmlspecialchars_decode( esc_attr( $slide['textslider_title'] ) ).'</h2></div>';
                    }
            ...
            case 'imageslider':
                ...
                if( !empty($imageslider_button1) ){
                    ...
                    $args = array( 
                        ...
                        'btn_icon_style' => $timageslider_button1_icon_style, 
                ...
                if( !empty($imageslider_button2) ){
                    ...
                    $args = array( 
                        ...
                        'btn_icon_style' => $timageslider_button2_icon_style, 
