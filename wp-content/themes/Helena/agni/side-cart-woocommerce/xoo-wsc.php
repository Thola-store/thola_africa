...
function xoo_wsc_init(){
	...
	define("XOO_WSC_PATH",get_template_directory() . '/agni/side-cart-woocommerce');
...
function get_cart_markup(){
	...
	require_once  get_template_directory() . '/agni/side-cart-woocommerce/xoo-wsc-markup.php';	
}
...
function update_cart_ajax(){
	...
	if(!$cart_key){
		wp_send_json(array('error' => __('Something went wrong','halena')));
	}
...
function enqueue_styles() {
	...
	wp_enqueue_style( $xoo_wsc, get_template_directory_uri() . '/agni/side-cart-woocommerce/css/xoo-wsc-public.css', array(), $version, 'all' );
...
$version = '1.0.2';
	...
	$ajax_atc  = 1; //isset( $gl_options['sc-ajax-atc']) ? $gl_options['sc-ajax-atc'] : 0;
	...
	wp_enqueue_script( $xoo_wsc, get_template_directory_uri() . '/agni/side-cart-woocommerce/js/xoo-wsc-public.js', array( 'jquery' ), $version, true );
...
if(!is_admin() || wp_doing_ajax()){
	add_action('agni_header_icons', 'get_cart_markup');
