		...
		<?php else: ?>
			...
			<?php foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
				...
				if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
					...
					$product_name =  apply_filters( 'woocommerce_cart_item_name', sprintf( '<h6 class="xoo-wsc-product-title cart-product-title"><a href="%s">%s</a></h6>', esc_url( $product_permalink ), $_product->get_title() ), $cart_item, $cart_item_key );
					...
					<div class="xoo-wsc-product" data-xoo_wsc="<?php echo esc_attr( $cart_item_key ); ?>">
						...
						<div class="xoo-wsc-sum-col">
							...
							<a href="#" class="xoo-wsc-remove"><?php esc_html_e('Remove','halena'); ?></a>
						</div>
					</div>
				<?php } ?>
			<?php } ?>
