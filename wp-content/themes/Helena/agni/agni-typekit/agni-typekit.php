...
{
	add_submenu_page('halena', esc_html__( 'Typekit Fonts', 'halena' ), esc_html__( 'TypeKit Fonts', 'halena' ), 'edit_theme_options', 'halena-typekit-font', 'agni_typekit_render_form' );
}
...
function agni_typekit_render_form()
{
	...
	?>
	<div class="wrap">
		<h1><?php esc_html_e( 'Typekit Fonts', 'halena' ) ?></h1>
		...
		<form method="post" action="options.php">
			...
			<div class="form-table">
				<h2 class="agni-typekit-text-field-title"><?php esc_html_e( 'Enter Typekit Kit ID', 'halena' ) ?></h2>
				...
				<p class="typekit-instruction"><?php echo wp_kses(__( 'See the instructions at <a href="#instructions" target="blank">bottom</a>', 'halena' ), array( 'a' => array( 'href' => array() ) ) ); ?></p>
			</div>
			<p class="agni-typekit-form-submit submit">
				<input type="submit" class="button button-primary button-large" value="<?php esc_attr_e( 'Save Typkit kit ID', 'halena' ) ?>" />
			</p>
		</form><br/>
	
	<?php if ( $agni_typekit_options['agni_typekit_id'] != '' ) { ?>
		<h3><?php echo esc_html_e( 'Font List :', 'halena' ); ?></h3>
		<p><?php echo esc_html_e( 'List of font you\'re using on Typekit.', 'halena' ); ?></p>
		...
		<thead>
			<tr>
				<?php echo '<th>'.esc_html__( 'Font', 'halena' ).'</th><th>'.esc_html__( 'Font Family', 'halena' ).'</th><th>'.esc_html__( 'Variations', 'halena' ).'</th><th>'.esc_html__( 'URL', 'halena' ).'</th>'; ?>
			</tr>
		</thead>
		<tfoot>
			<tr>
	...
	<h3><?php esc_html_e( 'Font usage :', 'halena' ); ?></h3>
	<p><?php echo sprintf( wp_kses( __( 'Add your desire CSS codes at <a href="%s">Custom CSS</a>', 'halena' ), array(  'a' => array( 'href' => array() ) ) ), esc_url( admin_url() . 'admin.php?page=halena-theme-options&tab=29' ) ); ?></p>
  ...
  font-style: italic;
	...
	<h3 id="instructions"><?php esc_html_e('Instructions', 'halena'); ?></h3>
	<ol>
		<li><?php echo wp_kses(__( 'Go to <a href="https://typekit.com/" target="blank">typekit.com</a> and register for an account', 'halena' ), array( 'a' => array( 'href' => array() ) ) ); ?></li>
		<li><?php esc_html_e('Choose a few fonts to add to your account and Publish them', 'halena'); ?></li>
		<li><?php esc_html_e('Click Kit Editor at the top right and get your Typekit kit ID(at the bottom)', 'halena'); ?></li>
	</ol>
