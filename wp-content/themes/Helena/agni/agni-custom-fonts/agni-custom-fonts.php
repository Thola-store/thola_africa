...
{
	add_submenu_page( 'halena', esc_html__( 'Custom Fonts', 'halena' ), esc_html__( 'Custom Fonts', 'halena' ), 'edit_theme_options', 'halena-custom-fonts', 'agni_custom_fonts' );
}
...
function agni_custom_fonts(){
	...
	<div class="wrap">
		<h1><?php esc_html_e( 'Custom Webfonts', 'halena' ); ?></h1>
		...
		<form action="admin.php?page=halena-custom-fonts" method="post" enctype="multipart/form-data">
			<div>
				<h2><?php esc_html_e( 'Enter Font Name', 'halena' ); ?></h2>
				<input type="text" name="text" required />
			</div><br />
			<div>
				<h3><?php esc_html_e( 'Upload Webfont Files', 'halena' ); ?></h3>
				<p><?php esc_html_e( 'Tip. you can add multiple files at once .eot, .woff, .svg, .ttf, .otf supported.', 'halena' ); ?></p>
		...
		<div>
			<h3><?php esc_html_e( 'Font List :', 'halena' ); ?></h3>
			<p><?php esc_html_e( 'List of fonts in Custom Fonts directory.', 'halena' ); ?></p>
		</div>
		
		<table class="widefat">
		<thead>
			<tr>
				<?php echo '<th>'.esc_html__( 'Font', 'halena' ).'</th><th>'.esc_html__( 'Font Family', 'halena' ).'</th><th>'.esc_html__( 'Manage', 'halena' ).'</th>'; 
				?>
			</tr>
		</thead>
		<tfoot>
		...
		<?php if ($handle = opendir($file_dirname)) {
		    ...
		    while (false !== ($file = readdir($handle))) {
		        if (!in_array($file, $blacklist)) {
					...
					<form action="admin.php?page=halenae-custom-fonts" method="post" >
		...
		<p><?php echo sprintf( wp_kses( __( 'Add your desire CSS codes at <a href="%s">Custom CSS</a>', 'halena' ), array(  'a' => array( 'href' => array() ) ) ), esc_url( admin_url() . 'admin.php?page=halena-theme-options&tab=29' ) );
		?></p>
  ...
  font-weight: normal;
		...
		<h3 id="instructions"><?php esc_html__('Instructions', 'halena'); ?></h3>
		<ol>
			<li><?php echo wp_kses(__( 'Generate the Webfont at <a href="https://www.web-font-generator.com/" target="blank">Web Font Generator</a>.', 'halena' ), array( 'a' => array( 'href' => array() ) ) ); ?></li>
			<li><?php esc_html_e('In case, you\'ve few variations of same font, you should upload it with different name.', 'halena'); ?></li>
		</ol>
