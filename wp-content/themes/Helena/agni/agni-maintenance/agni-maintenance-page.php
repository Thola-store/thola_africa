...
function agni_maintenance_wp_die_handler( $message, $title = '', $args = array() ) {
    ...
    if ( isset( $r['back_link'] ) && $r['back_link'] ) {
        $back_text = $have_gettext? __('&laquo; Back', 'halena') : '&laquo; Back';
    ...
    if ( ! did_action( 'admin_head' ) ) :
        ...
        if ( empty($title) )
            $title = $have_gettext ? __('WordPress &rsaquo; Error', 'halena') : 'WordPress &rsaquo; Error';
...
<head>
    ...
    ?>
    <?php wp_title(); ?>
    <link rel="stylesheet" href="<?php echo esc_url( AGNI_FRAMEWORK_CSS_URL . '/halena.css' ); ?>" type="text/css" media="all" />   
...
<?php endif; // ! did_action( 'admin_head' ) ?>
    ...
    if( $halena_options['maintenance-mode-choice'] == '1' ){
        ...
        <div id="content" class="agni-maintenance-mode-content">
            ...
            <h2>'.esc_html__( 'We\'ll be right back!', 'halena' ).'</h2>
            <p>'.esc_html__( 'Sorry for the inconvenience. We\'re busy on making something cool for you.', 'halena' ).'<br/>'.esc_html__( 'Please try after sometime.', 'halena' ).'</p>
        </div>';
    }
    else{
