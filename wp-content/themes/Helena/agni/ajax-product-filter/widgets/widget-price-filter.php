	...
	class WCAPF_Price_Filter_Widget extends WP_Widget {
		...
		function __construct() {
			parent::__construct(
				...
				__('WC Ajax Product Filter by Price', 'halena'), // Name
				array('description' => __('Filter woocommerce products by price.', 'halena')) // Args
			);
		}
		...
		public function form($instance) {
			...
			<p>
				...
				<select class="widefat price-filter-display-type" id="<?php echo esc_attr( $this->get_field_id('display_type') ); ?>" name="<?php echo esc_attr( $this->get_field_name('display_type') ); ?>">
					<option value="slider" <?php echo ((!empty($instance['display_type']) && $instance['display_type'] === 'slider') ? 'selected="selected"' : ''); ?>><?php printf(__('Slider', 'halena')); ?></option>
			...
			<p class="price-list-currency <?php echo (!isset($instance['display_type']) || $instance['display_type'] === 'slider') ? 'hidden' : ''; ?>">
				...
				<label for="<?php echo esc_attr( $this->get_field_id('show_currency') ); ?>"><?php printf(__('Show currency', 'halena')); ?></label>
			</p>
			<div class="price-list-wrapper <?php echo (!isset($instance['display_type']) || $instance['display_type'] === 'slider') ? 'hidden' : ''; ?>">
				<?php if (isset($instance['price_list']) && !empty($instance['price_list'])): ?>
					<?php foreach ($instance['price_list'] as $price_list): ?>
						<p class="price-list">
							<input type="text" class="widefat min" name="<?php echo esc_attr( $this->get_field_name('price_list') ); ?>[min][]" value="<?php echo esc_attr( $price_list['min'] ); ?>" placeholder="<?php printf(__('Min price', 'halena')); ?>" />
							<input type="text" class="widefat to" name="<?php echo esc_attr( $this->get_field_name('price_list') ); ?>[to][]" value="<?php echo esc_attr( $price_list['to'] ); ?>" placeholder="<?php printf(__('to', 'halena')); ?>" />
							<input type="text" class="widefat max" name="<?php echo esc_attr( $this->get_field_name('price_list') ); ?>[max][]" value="<?php echo esc_attr( $price_list['max'] ); ?>" placeholder="<?php printf(__('Max price', 'halena')); ?>" />
							<a href="javascript:void(0)" class="remove-price-list">&times;</a>
						</p>
					<?php endforeach ?>
				<?php else: ?>
					<p class="price-list">
						<input type="text" class="widefat min" name="<?php echo esc_attr( $this->get_field_name('price_list') ); ?>[min][]" value="" placeholder="<?php printf(__('Min price', 'halena')); ?>" />
						<input type="text" class="widefat to" name="<?php echo esc_attr( $this->get_field_name('price_list') ); ?>[to][]" value="" placeholder="<?php printf(__('to', 'halena')); ?>" />
						<input type="text" class="widefat max" name="<?php echo esc_attr( $this->get_field_name('price_list') ); ?>[max][]" value="" placeholder="<?php printf(__('Max price', 'halena')); ?>" />
						<a href="javascript:void(0)" class="remove-price-list">&times;</a>
					</p>
				<?php endif ?>
			</div>
			<p class="add-price-list-button-wrapper <?php echo (!isset($instance['display_type']) || $instance['display_type'] === 'slider') ? 'hidden' : ''; ?>">
				<a href="javascript:void(0)" class="button add-price-list"><?php printf(__('Add', 'halena')); ?></a>
			</p>
