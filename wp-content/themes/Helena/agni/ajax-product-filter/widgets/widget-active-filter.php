	...
	class WCAPF_Active_Filters_Widget extends WP_Widget {
		...
		function __construct() {
			parent::__construct(
				...
				__('WC Ajax Active Filters', 'halena'), // Name
				array('description' => __('Shows active filters so users can see and deactivate them.', 'halena')) // Args
			);
		}
			...
			if (sizeof($active_filters) > 0) {
				...
				<div class="wcapf-active-filters">
					...
					<?php foreach ($active_filters as $key => $active_filter) {
						...
						if ($key === 'keyword') { ?>
							<a href="javascript:void(0)" data-key="keyword"><?php esc_html_e( 'Search For: ', 'halena'); echo esc_html( $active_filter ); ?></a>
						<?php }

						if ($key === 'orderby') { ?>
							<a href="javascript:void(0)" data-key="orderby"><?php esc_html_e( 'Orderby: ', 'halena'); echo esc_html( $active_filter ); ?></a>
						<?php }

						if ($key === 'min_price') { ?>
							<a href="javascript:void(0)" data-key="min-price"><?php esc_html_e( 'Min Price: ', 'halena'); echo esc_html( $active_filter ); ?></a>
						<?php }

						if ($key === 'max_price') { ?>
							<a href="javascript:void(0)" data-key="max-price"><?php esc_html_e( 'Max Price: ', 'halena'); echo esc_html( $active_filter ); ?></a>
						<?php }
		...
		public function form($instance) {
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id('button_text') ); ?>"><?php printf(__('Button Text:', 'halena')); ?></label>
