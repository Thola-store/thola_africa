...
class TA_WC_Variation_Swatches_Admin {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_style( 'tawcvs-admin', AGNI_THEME_FILES_URL . '/variation-swatches-for-woocommerce/assets/css/admin.css', array( 'wp-color-picker' ), '20160615' );
		wp_enqueue_script( 'tawcvs-admin', AGNI_THEME_FILES_URL . '/variation-swatches-for-woocommerce/assets/js/admin.js', array( 'jquery', 'wp-color-picker', 'wp-util' ), '20170113', true );
		...
		wp_localize_script(
			...
			array(
				'i18n'        => array(
					'mediaTitle'  => esc_html__( 'Choose an image', 'halena' ),
					'mediaButton' => esc_html__( 'Use image', 'halena' ),
				),
		...
		switch ( $type ) {
			case 'image':
				...
				<div style="line-height:60px;">
					...
					<button type="button" class="tawcvs-upload-image-button button"><?php esc_html_e( 'Upload/Add image', 'halena' ); ?></button>
					<button type="button" class="tawcvs-remove-image-button button <?php echo ( $value ) ? '' : 'hidden' ?>"><?php esc_html_e( 'Remove image', 'halena' ); ?></button>
				</div>
				<?php
				break;
	...
	public function product_option_terms( $taxonomy, $index ) {
		...
		<select multiple="multiple" data-placeholder="<?php esc_attr_e( 'Select terms', 'halena' ); ?>" class="multiselect attribute_values wc-enhanced-select" name="attribute_values[<?php echo esc_attr( $index ); ?>][]">
			<?php
		...
		<button class="button plus select_all_attributes"><?php esc_html_e( 'Select all', 'halena' ); ?></button>
		<button class="button minus select_no_attributes"><?php esc_html_e( 'Select none', 'halena' ); ?></button>
		<button class="button fr plus tawcvs_add_new_attribute" data-type="<?php echo esc_attr( $taxonomy->attribute_type ) ?>"><?php esc_html_e( 'Add new', 'halena' ); ?></button>
	...
	public function add_attribute_term_template() {
		...
		<div id="tawcvs-modal-container" class="tawcvs-modal-container">
			<div class="tawcvs-modal">
				...
				<div class="tawcvs-modal-footer">
					<button class="button button-secondary tawcvs-modal-close"><?php esc_html_e( 'Cancel', 'halena' ) ?></button>
					<button class="button button-primary tawcvs-new-attribute-submit"><?php esc_html_e( 'Add New', 'halena' ) ?></button>
		...
		<p id="tmpl-tawcvs-input-color">

			<label><?php esc_html_e( 'Color', 'halena' ) ?></label><br>
			<input type="text" class="tawcvs-input tawcvs-input-color" name="swatch">

		</p>

		<p id="tmpl-tawcvs-input-image">

			<label><?php esc_html_e( 'Image', 'halena' ) ?></label><br>
			<div class="tawcvs-term-image-thumbnail" style="float:left;margin-right:10px;">
			...
			<div style="line-height:60px;">
				...
				<button type="button" class="tawcvs-upload-image-button button"><?php esc_html_e( 'Upload/Add image', 'halena' ); ?></button>
				<button type="button" class="tawcvs-remove-image-button button hidden"><?php esc_html_e( 'Remove image', 'halena' ); ?></button>
			</div>

		</p>

		<p id="tmpl-tawcvs-input-label">

			<label>
				<?php esc_html_e( 'Label', 'halena' ) ?>
	...
	public function add_new_attribute_ajax() {
		...
		if ( ! wp_verify_nonce( $nonce, '_tawcvs_create_attribute' ) ) {
			wp_send_json_error( esc_html__( 'Wrong request', 'halena' ) );
		}

		if ( empty( $name ) || empty( $swatch ) || empty( $tax ) || empty( $type ) ) {
			wp_send_json_error( esc_html__( 'Not enough data', 'halena' ) );
		}

		if ( ! taxonomy_exists( $tax ) ) {
			wp_send_json_error( esc_html__( 'Taxonomy is not exists', 'halena' ) );
		}

		if ( term_exists( $_POST['name'], $_POST['tax'] ) ) {
			wp_send_json_error( esc_html__( 'This term is exists', 'halena' ) );
		}
		...
		wp_send_json_success(
			array(
				'msg'  => esc_html__( 'Added successfully', 'halena' ),
				'id'   => $term->term_id,
				'slug' => $term->slug,
				'name' => $term->name,
			)
